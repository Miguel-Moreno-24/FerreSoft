...
function validateAccountForm() {
  ...
  if (!/^[\p{L}\s'-]+$/u.test(nombre)) return 'El nombre solo puede contener letras y espacios'

  if (!email) return 'Debes ingresar un correo electrónico'
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return 'Ingresa un correo válido'

  if ((tipoDocumento && !numeroDocumento) || (!tipoDocumento && numeroDocumento)) {
    return 'Debes completar tipo y número de documento juntos'
  }

  if (tipoDocumento && !/^[0-9A-Za-z-]{5,20}$/.test(numeroDocumento)) {
    return 'El número de documento debe tener entre 5 y 20 caracteres válidos'
  }

  if (telefono && !/^\+?[0-9\s-]{7,20}$/.test(telefono)) {
    return 'El teléfono debe tener entre 7 y 20 caracteres numéricos'
  }
...
async function loadUserData() {
  try {
    ...
    if (!data.success) {
      showToast(data.message || 'Error cargando los datos', 'error')
      setTimeout(() => {
...
document.getElementById('account-form')?.addEventListener('submit', async (event) => {
  ...
  showConfirm('¿Guardar cambios en tu cuenta?', async () => {
    ...
    try {
      ...
      showAccountMessage('Error al guardar los cambios', 'error')
    }
  })
})
