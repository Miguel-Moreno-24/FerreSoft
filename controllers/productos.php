...
function validateProductData(array &$errors, string &$nombre, string &$descripcion, &$precio, &$stock, string &$imagen): void
{
    $nombre = trim($nombre);
    if ($nombre === '' || !preg_match('/^[\p{L}\s]{1,60}$/u', $nombre)) {
    ...
    if ($imagen !== '' && strpos($imagen, 'uploads/') !== 0 && !filter_var($imagen, FILTER_VALIDATE_URL)) {
        $errors[] = 'La URL de la imagen no es válida.';
    }
}

function handleUploadedImage(string &$imagen, array &$errors): void
{
    ...
    $appBase = rtrim(str_replace('/controllers', '', $scriptDir), '/');
...
switch ($action) {
    ...
    case 'add':
        ...
        $newProductId = appGetNextExistingId($conn, 'productos');
        $stmt = $conn->prepare('INSERT INTO productos (id, nombre, descripcion, precio, stock, imagen) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->bind_param('issdis', $newProductId, $nombre, $descripcion, $precio, $stock, $imagen);
